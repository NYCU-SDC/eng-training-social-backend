...
func (s *Service) GetByPostIDAndUserID(ctx context.Context, postID, userID uuid.UUID) (ReactionType, error) {
	exists, err := s.queries.ExistsByPostIDAndUserID(ctx, ExistsByPostIDAndUserIDParams{
		PostID: pgtype.UUID{Valid: true, Bytes: postID},
		UserID: userID,
	})
	if err != nil {
		s.logger.Error("Failed to get reaction", zap.Error(err))
		return ReactionTypeNONE, err
	}

	if !exists {
		s.logger.Info("No reaction found for post", zap.String("postID", postID.String()), zap.String("userID", userID.String()))
		return ReactionTypeNONE, nil
	}
...
func (s *Service) GetByCommentIDAndUserID(ctx context.Context, commentID, userID uuid.UUID) (ReactionType, error) {
	exists, err := s.queries.ExistsByCommentIDAndUserID(ctx, ExistsByCommentIDAndUserIDParams{
		CommentID: pgtype.UUID{Valid: true, Bytes: commentID},
		UserID:    userID,
	})
	if err != nil {
		s.logger.Error("Failed to check existence of reaction", zap.Error(err))
		return ReactionTypeNONE, err
	}

	if !exists {
		s.logger.Info("No reaction found for comment", zap.String("commentID", commentID.String()), zap.String("userID", userID.String()))
		return ReactionTypeNONE, nil
	}
